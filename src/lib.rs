    ...
    match platform {
        "windows" => {
            #[cfg(target_os = "windows")]
            {
                ...
                let hinstance =
                    unsafe { NonZeroIsize::new(GetModuleHandleW(std::ptr::null()) as isize) };
        ...
        "linux" => {
            #[cfg(target_os = "linux")]
            {
                ...
                let dpy =
                    XlibDisplayHandle::new(NonNull::new(display_id as *mut std::ffi::c_void), 0);
...
> {
    ...
    let positions_cpu: Vec<[f32; 3]> = if let Ok(s) = positions.as_slice() {
        ...
        (0..n)
            .map(|i| [pos_array[[i, 0]], pos_array[[i, 1]], pos_array[[i, 2]]])
            .collect()
    };
    ...
    enum ColorData<'a> {
        Rgb {
            slice: Option<&'a [[f32; 3]]>,
            owned: Option<Vec<[f32; 3]>>,
        },
        Scalar {
            slice: Option<&'a [f32]>,
    ...
    let color_data: ColorData<'_> = if let Some(ref rgb) = colors {
        ...
        match rgb.as_slice() {
            Ok(s) => ColorData::Rgb {
                slice: Some(bytemuck::cast_slice(s)),
                owned: None,
            },
            Err(_) => ColorData::Rgb {
                slice: None,
    ...
    let instances: Vec<PointInstance> = match &color_data {
        ColorData::ZDefault { cpts } => {
            ...
            flat_pos
                ...
                .enumerate()
                .map(|(i, p)| {
                    let color = normalize(p[2], vmin, range)
                        .map(|t| colormap::sample(cpts, t))
                        .unwrap_or(nan_col);
                    PointInstance {
                        position: *p,
                        size: pt_size(i),
...
fn build_line_vertices(
    ...
    for i in 0..rows {
        verts.push(grid::LineVertex {
            position: [arr[[i, 0]], arr[[i, 1]], arr[[i, 2]]],
            color,
        });
        verts.push(grid::LineVertex {
            position: [arr[[i, 3]], arr[[i, 4]], arr[[i, 5]]],
            color,
        });
    }
    Ok(verts)
...
impl PyScatterRenderer {
    ...
    fn refresh_scene_bounds(&mut self, fit_if_unfitted: bool) {
        if let Some((union_min, union_max)) = self.inner.actor_union_bounds() {
            ...
            self.inner
                .set_grid(union_min, union_max, nice_min, nice_max);
...
impl PyScatterRenderer {
    ...
    fn set_points(
        ...
        let handle = self
            .inner
            .set_points(&instances, pos_cpu, n as u32, bmin, bmax);
    ...
    fn add_points(
        ...
        let handle = self
            .inner
            .add_actor(&instances, pos_cpu, n as u32, bmin, bmax);
    ...
    fn update_actor(
        ...
        match result {
            ...
            Some((instances, pos_cpu, n, bmin, bmax)) => {
                self.inner
                    .update_actor_data(handle, &instances, pos_cpu, n as u32, bmin, bmax);
            }
        }
    ...
    fn create_stream(
        ...
        let handle = self.inner.create_stream(
            max_points,
            stream_mode,
            &instances,
            pos_cpu,
    ...
    fn stream_points(
        ...
        let (instances, pos_cpu, bmin, bmax) = match build_instances(
            positions,
            colors,
            scalars,
            point_sizes,
        ...
        let (ok, bounds_grew) = self
            .inner
            .append_to_stream(handle, &instances, &pos_cpu, bmin, bmax);
    ...
    fn view_xy(&mut self) {
        self.inner
            .set_view_direction(0.0, std::f32::consts::FRAC_PI_2 - 0.001);
    }
    ...
    fn view_yz(&mut self) {
        self.inner
            .set_view_direction(-std::f32::consts::FRAC_PI_2, 0.0);
    }

    /// Isometric-style view: equal footing on all three axes.
    fn view_isometric(&mut self) {
        self.inner
            .set_view_direction(std::f32::consts::FRAC_PI_4, std::f32::consts::FRAC_PI_4);
    }
    ...
    fn chart2d_add_line(
        ...
        Ok(self
            .inner
            .chart2d_add_line(xs, ys, [color.0, color.1, color.2], line_width))
    }
    ...
    fn chart2d_update_line(
        ...
        let xs = x.as_slice().map_err(|_| {
            pyo3::exceptions::PyValueError::new_err(
                "x must be a contiguous array; call np.ascontiguousarray(x) first",
            )
        })?;
        let ys = y.as_slice().map_err(|_| {
            pyo3::exceptions::PyValueError::new_err(
                "y must be a contiguous array; call np.ascontiguousarray(y) first",
            )
        })?;
        Ok(self
            .inner
            .chart2d_update_line(handle, xs, ys, [color.0, color.1, color.2], line_width))
    }
    ...
    fn chart2d_set_line_markers(
        ...
        self.inner
            .chart2d_set_line_markers(handle, style, size_px, every)
    }
    ...
    fn chart2d_add_hspan(&mut self, y0: f32, y1: f32, color: (f32, f32, f32, f32)) -> u32 {
        self.inner
            .chart2d_add_hspan(y0, y1, [color.0, color.1, color.2, color.3])
    }
    ...
    fn chart2d_add_vspan(&mut self, x0: f32, x1: f32, color: (f32, f32, f32, f32)) -> u32 {
        self.inner
            .chart2d_add_vspan(x0, x1, [color.0, color.1, color.2, color.3])
    }
    ...
    fn chart2d_add_hline(&mut self, y: f32, color: (f32, f32, f32), line_width: f32) -> u32 {
        self.inner
            .chart2d_add_hline(y, [color.0, color.1, color.2], line_width)
    }
    ...
    fn chart2d_add_vline(&mut self, x: f32, color: (f32, f32, f32), line_width: f32) -> u32 {
        self.inner
            .chart2d_add_vline(x, [color.0, color.1, color.2], line_width)
    }
    ...
    fn chart2d_set_legend(
        ...
        let entries_rust: Vec<(String, [f32; 3])> = entries
            .into_iter()
            .map(|(lbl, (r, g, b))| (lbl, [r, g, b]))
            .collect();
        self.inner
            .chart2d_set_legend(entries_rust, position, visible);
    }
    ...
    fn set_camera(&mut self, state: &pyo3::Bound<'_, pyo3::types::PyDict>) -> PyResult<()> {
        ...
        let target_list: Vec<f32> = state
            .get_item("target")?
            .ok_or_else(|| PyRuntimeError::new_err("missing key: target"))?
            .extract()?;
        ...
        let cs = CameraState {
            ...
            distance: state
                .get_item("distance")?
                .ok_or_else(|| PyRuntimeError::new_err("missing key: distance"))?
                .extract()?,
            yaw: state
                .get_item("yaw")?
                .ok_or_else(|| PyRuntimeError::new_err("missing key: yaw"))?
                .extract()?,
            pitch: state
                .get_item("pitch")?
                .ok_or_else(|| PyRuntimeError::new_err("missing key: pitch"))?
                .extract()?,
            parallel: state
                .get_item("parallel")?
                .ok_or_else(|| PyRuntimeError::new_err("missing key: parallel"))?
                .extract()?,
        };
    ...
    fn show_scalar_bar(
        ...
        self.inner
            .set_scalar_bar(visible, vmin, vmax, log_scale, cpts, title);
    }
    ...
    fn pick_point(
        ...
        self.inner
            .pick_point(screen_x, screen_y)
            .map(|(actor_id, idx, pos)| {
                Python::with_gil(|py| {
    ...
    fn set_ticks(&mut self, x: Option<usize>, y: Option<usize>, z: Option<usize>) {
    ...
    fn set_axis_labels(&mut self, x: &str, y: &str, z: &str) {
        self.inner
            .set_axis_labels(x.to_string(), y.to_string(), z.to_string());
    }
    ...
    fn add_user_label(
        ...
        self.inner
            .add_user_label(x, y, z, text, color, size, anchor)
    }
    ...
    fn update_user_label(
        ...
        self.inner
            .update_user_label(id, pos, text, color, size, anchor);
    }
    ...
    fn add_mesh(
        ...
        if idx.ncols() != 3 {
            return Err(pyo3::exceptions::PyValueError::new_err(format!(
                "indices must have shape (N, 3), got {:?}",
                idx.shape()
            )));
        }
        let nv = v.nrows();
    ...
    fn update_mesh(
        ...
        let idxs: Vec<[u32; 3]> = (0..nf)
            ...
            .collect();
        self.inner
            .update_mesh_actor(handle, &verts, &idxs, color, wireframe);
        self.refresh_scene_bounds(false);
        Ok(())
    }
